...
This table allows the filtering shows the number of work types per sub-background message. It allows filtering as well as sortu=ing and downloading.
...
In the following tables only the papers which occur in more than one background message are included.

### Key papers

The key papers include al key papers.
...
### All papers excluding Key Papers

Here we are only interested in papers which were published after 2018, as we eant to discover new evidence.

```{r}
#| label: non_key_paper_overlap
#|

fn <- file.path("output", "non_key_paper_overlap.rds")

if (!file.exists(fn)) {
  overlap_non_keypapers <- nodes |>
    dplyr::filter(
      !oa_input,
      !is.na(id),
      publication_year > 2018
